...
- Changes to telemetry env vars (`GRAFANA_OTLP_USERNAME`/`PASSWORD`/`BASIC_AUTH`, `OTEL_TRACE_RATIO`, `OTEL_EXPORTER_OTLP_*`, `NEXT_PUBLIC_OTEL_PROXY_URL`, `OBSERVABILITY_DEPLOYMENT_SECRET`) require synchronized updates to `docs/runbooks/observability-operations.md` and ADR-004 so on-call engineers can redeploy safely.
