...
## Definition of Ready
- [x] Data contracts confirmed: read models aggregating subscriptions (Stripe→Supabase), diagnostics (Ops Hub), recommendations engine, and mailing prefs (Listmonk) with versioned `v1` DTOs.
...
- [x] API backlog drafted: endpoints `GET /me`, `GET /me/subscriptions`, `POST /me/export`, `POST /me/delete`, `GET /me/downloads` plus Supabase migrations for `mailing_preferences` and `entitlement_views`.
