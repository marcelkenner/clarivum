...
`;
  ...
  const playwrightReportTemplate = `# playwright-report · AGENTS Guide

HTML artifacts generated by Playwright's reporters land here (e.g., \`npm run test:e2e:smoke\`). Treat the directory as disposable output—never hand-edit the files.

## Commands & upkeep

- Generate a fresh report after running a suite: \`PLAYWRIGHT_BASE_URL=http://127.0.0.1:3310 npm run test:e2e:smoke\`. (Start the dev server separately via \`npm run dev -- --hostname 127.0.0.1 --port 3310\`.)
- Open the latest report without rerunning tests: \`npx playwright show-report\`.
- Clean stale HTML before committing to avoid bloating diffs: \`rm -rf playwright-report && npm run test:e2e:smoke\`.

## Best practices

- Do not check these artifacts into git; share traces/screenshots by zipping the folder or linking to CI artifacts.
- Keep documentation about failures in runbooks or issues (not in the generated HTML).
- Update \`playwright.config.ts\` if you add new reporters and reflect changes here.

Always consult Context7 for Playwright API changes before altering the workflow.
`;

  const playwrightResultsTemplate = `# playwright-results · AGENTS Guide

This directory stores Playwright traces, screenshots, and videos (configured via \`playwright.config.ts#outputDir\`). The files are ephemeral and should be regenerated, not edited.

## How to use

- Smoke suite for QA metrics: run \`npm run dev -- --hostname 127.0.0.1 --port 3310\` in one terminal, then \`PLAYWRIGHT_BASE_URL=http://127.0.0.1:3310 npm run test:e2e:smoke\`.
- Regression suite: \`npm run test:e2e:regression\`.
- Clean artifacts when switching branches: \`rm -rf playwright-results\`.

## Tips

- When sharing evidence, attach only the relevant traces/videos to the issue or Sisu note.
- Do not edit files here to "fix" metrics—rerun the suite so the reporter re-emits \`metrics/quality.json\`.
- Keep this directory out of PRs; double-check \`.gitignore\` if new subfolders appear.
`;
  ...
  const templatesMap = new Map([
    ...
    ["src/app", appTemplate],
    ["tasks", tasksTemplate],
    ["playwright-report", playwrightReportTemplate],
    ["playwright-results", playwrightResultsTemplate],
  ]);
