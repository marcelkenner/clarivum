...
## Overview
These diagrams describe the Listmonk-based mailing stack, including infrastructure on AWS, synchronization with Aurora, and automation services that coordinate campaigns. They show how audiences flow between systems, the service classes that orchestrate Listmonk APIs, and the operational lifecycle for campaign execution.

## Files
- `architecture-overview.mmd` — ECS Fargate deployment, integrations, and feedback loops.
- `data-lineage.mmd` — Subscriber records, consent flags, and event tracking across Aurora and Listmonk.
