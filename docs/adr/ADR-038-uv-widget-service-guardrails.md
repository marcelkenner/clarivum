...
- Platform-wide guardrails dictate that caching and throttling rely on the shared AWS ElastiCache Serverless platform (ADR-006) and that analytics flow exclusively through Plausible (ADR-029).
...
- Replace the local cache and rate limiter with the Cache Gateway + ElastiCache Serverless (token bucket + sliding window scripts), enforcing a 5 minute payload cache (keyed by locale and geo roundings) and 30 req/min/IP throttling by default.
...
- Additional implementation work is required before launch (Strapi schema, Cache Gateway integration, analytics dashboards), but the tasks are scoped and tracked.
- Future tools can reuse the same pattern (external source + Strapi + Cache Gateway + Plausible) by extending this ADR, increasing consistency across the Tools platform.

## Related Tasks
- `tasks/backlog/platform/plat-047-uv-widget-strapi-copy.md`
- `tasks/backlog/platform/plat-048-uv-widget-cache-gateway.md`
