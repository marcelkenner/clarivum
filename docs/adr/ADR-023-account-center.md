...
- We must comply with GDPR (data portability, deletion), integrate with Aurora-backed auth data (ADR-001 & ADR-002), and surface personalization outputs (diagnostics, recommendations).
- Prior architectures risked scattering user data across services, complicating compliance and UX.

## Decision
- Implement a **Profile Service Layer** backed by Aurora PostgreSQL.
  - Use Aurora Row Level Security for tenant isolation (ADR-001).
  - Synchronize Auth0 user metadata with Aurora profiles; maintain hashed identifiers for analytics (ADR-029).
...
## Diagrams
- [Architecture Overview](../diagrams/adr-023-account-center/architecture-overview.mmd) — Auth0 synchronization, Aurora profile service, integrations, and frontend consumption.
...
- **Follow-ups:**
  - Define Aurora DB schema and migrations for profile entities.
