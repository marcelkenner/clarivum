...
- **Przelewy24:** Merchant IDs & POS IDs stored in Secrets Manager; redirect URLs point to AWS API Gateway → ECS handlers for webhook verification (reference `/websites/developers_przelewy24_pl` transaction registration).

## Provider onboarding checklist
### Stripe
1. **Create connected user:** Add operator in Stripe Dashboard with `Read & Write` access for payments and refunds.
2. **Verify webhooks:** Ensure the AWS API Gateway webhook endpoint is subscribed to `payment_intent.succeeded`, `payment_intent.payment_failed`, `charge.refunded`.
...
1. **Register transaction flow:** Configure `/api/transactions` per Context7 docs with `urlReturn` and `urlStatus` hitting the AWS API Gateway → ECS handler.
...
2. **Domain verification:** Upload the `apple-developer-merchantid-domain-association` file to the CloudFront origin bucket for each storefront domain; verify in Stripe and Apple Developer account. For Google Pay, confirm domain is registered in Stripe and Przelewy24 per redundancy plan.
...
- **Weekly:** Reconcile payouts vs Aurora ledger (see next section).
...
2. **Normalize:** Load into Aurora reporting tables via `npm run payments:import -- --provider stripe`.
...
## Refunds & adjustments
1. **Stripe refunds:** Trigger via admin panel or CLI (`stripe refunds create --charge ch_xxx`). Update Aurora `payment_status` to `refunded`; notify customer via transactional email (Listmonk template).
2. **PayU/Przelewy24 refunds:** Initiate through provider dashboard API (requires token). Document refund reference in Aurora `provider_metadata`.
...
- Store only provider IDs and status in Aurora; never persist raw PAN/BLIK codes.
