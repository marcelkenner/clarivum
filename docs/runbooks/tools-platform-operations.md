...
- Rate limiting: default 30 req/min per hashed client IP using `@upstash/ratelimit` (`UPSTASH_RATELIMIT_REST_URL`/`TOKEN`). Metrics emit under `clarivum.tools.uv_widget.rate_limit.*`. Enable fleet-wide throttling with `UV_WIDGET_GLOBAL_RATE_LIMIT_PER_MIN` when marketing pushes run; alerts must cover both scopes.
