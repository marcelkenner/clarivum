...
- Amazon S3 buckets holding PDF/EPUB assets.
...
- Pending profile state (`pending_claim`) enabled in Aurora with claim tokens ready for Auth0 verification.
- S3 object paths versioned (`ebooks/<slug>/<version>/file.pdf`).
- Support CRM macros prepared for ebook issues.

## Tooling & References
- Stripe Dashboard (payments, refunds).
- AWS Console (S3 object explorer, Aurora query editor).
...
- Spot-check S3 signed URLs expiry and access logs.
...
### Monthly
- Reconcile revenue vs Aurora entitlement table counts.
...
2. `EbookFulfillmentManager` records purchase in Aurora (`ebook_entitlements` table).
...
5. Profile library displays entitlements via Aurora view.
6. Scheduled reminder email (24h) prompts user to download if unopened.

## Support Procedures
### Resend Download Link
- Locate entitlement in Aurora (query by email/order ID).
...
- Confirm entitlement ownership (email, order ID) and check status in Aurora (`entitlement_status`).
...
### Failed Download
- Validate S3 object exists and ACL allows authenticated user.
...
- Update Aurora entitlement status to `revoked`.
...
### Storage Outage
- Confirm S3 status (AWS Health dashboard); if regional outage, post maintenance message on site.
...
- Run reconciliation script comparing Stripe payments vs Aurora entitlements.
