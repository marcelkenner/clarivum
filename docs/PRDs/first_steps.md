...
- **Dev environment status (2025-10-30):** Track A is provisioned in AWS account `869603330574`—CloudFront `EPHSANK5PAPBA` (alias `dev.clarivum.com`), S3 buckets `clarivum-dev-static-869603330574` / `clarivum-dev-media-869603330574`, API Gateway `j0cjdyuqti` + Lambda `platform-dev-core`, DynamoDB `platform-dev-kv`, and Aurora `platform-dev-aurora`; CMS remains Strapi on ECS (no Lightsail workload created).
