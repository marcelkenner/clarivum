...
- UV widget cache & throttling rely on ElastiCache Serverless (`UV_WIDGET_CACHE_ENDPOINT`, `UV_WIDGET_CACHE_PORT`, `UV_WIDGET_CACHE_USE_TLS`) with the shared Redis connection (`UV_WIDGET_RATE_LIMIT_MODE=redis`). For maintenance or local dev, set `UV_WIDGET_CACHE_MODE=memory` and `UV_WIDGET_RATE_LIMIT_MODE=memory`. Global throttling defaults to `UV_WIDGET_GLOBAL_RATE_LIMIT_PER_MIN=0`; raise it per environment when platform signs off.
